      ...
      <div className="inline-flex bg-gray-100/80 dark:bg-gray-900/80 p-1.5 rounded-2xl mb-6 border border-gray-200/60 dark:border-gray-700/60 shadow-inner">
        {(['Produtos', 'Adicionais'] as const).map(tab => (
          <button
            ...
            className={`px-8 py-2.5 rounded-xl font-bold text-sm transition-all duration-300 whitespace-nowrap ${activeTab === tab
                  ...
                  <div className="flex flex-wrap gap-4 items-start">
                    <Reorder.Group axis="x" values={images} onReorder={setImages} className="flex flex-wrap gap-4">
                      {images.map((img, index) => (
                        ...
                        >
                          <div className="absolute -top-3 -left-3 bg-vanta-blue text-white text-xs font-bold w-7 h-7 flex items-center justify-center rounded-full z-10 shadow-md border-2 border-white dark:border-gray-900">
                            {index + 1}
                          </div>
