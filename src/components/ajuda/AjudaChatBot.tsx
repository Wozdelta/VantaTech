...
export default function AjudaChatBot({ onOpenTicket }: { onOpenTicket: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-800">
      ...
      <div 
        ref={scrollRef}
        className="flex-1 min-h-0 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6 custom-scrollbar"
      >
        <AnimatePresence initial={false}>
          {messages.map((msg) => (
            <motion.div 
              ...
              key={msg.id} 
              className={`flex gap-2 md:gap-4 ${msg.type === 'user' ? 'justify-end' : 'justify-start'} group`}
            >
              {msg.type === 'bot' && (
                <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br from-vanta-blue to-blue-600 rounded-full flex items-center justify-center flex-shrink-0 mt-1 shadow-sm">
                  <Bot className="w-4 h-4 md:w-5 md:h-5 text-white" />
                </div>
              )}
              
              <div className="flex flex-col gap-1 max-w-[85%] md:max-w-[80%] min-w-0">
              ...
              {msg.type === 'user' && (
                <div className="w-8 h-8 md:w-10 md:h-10 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center flex-shrink-0 mt-1 shadow-sm">
                  <User className="w-4 h-4 md:w-5 md:h-5 text-gray-500 dark:text-gray-300" />
                </div>
              )}
            </motion.div>
          ))}
